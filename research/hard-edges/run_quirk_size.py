...
for c in cands:
    ...
    try:
        ...
        taddr = twin[2]
        try:
            _liq = call("base", taddr, "liquidity()", out=("uint128",))
        except Exception: _liq = None
        if not _liq or _liq[0] == 0: print("skip", c["poolId"][:12], "twin has no liquidity"); continue
        ...
        print('ERR', c['poolId'][:12], str(ex)[:120]); sys.stdout.flush()
